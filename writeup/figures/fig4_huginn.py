...
set of flip loops only 8% of the time (ARC-Easy, K=30; ARC-Challenge K=32: 4%; RESULT 2 reseed control).
B. Every question at K=64 as a strip over loops: shaded where the current leader differs
from the final one, sorted by the loop of the last flip. K=16 and K=30 are literally
prefixes of this run (RESULT 11), marked as vertical lines. Below: mean margin between
the top two options, which stops growing where the flips stop.
...
LEAST overlap of flip loops between restarts (so the reseed scatter is visible).
"""
...
title(ax, "one ARC-Easy question, three random initial states: an early wobble, then the same letter")
ax.legend(loc="upper right", fontsize=7)
ax.text(0.0, -0.13, wrap(ax, "ARC-Easy, 50 questions, 30 loops: restarts end on the same letter 96% of the time and share the exact set of flip loops 8% of the time (ARC-Challenge, 32 loops: 4%). Shaded: loops before a letter is the next token (the four letters carry <5% of the mass there).", 6.8),
...
hx.set_yticks([]); hx.set_ylabel(f"{len(rows)} questions, sorted by their last flip")
title(hx, "all 50 ARC-Easy questions over 64 loops (red: current leader ≠ final answer); K=16 and K=30 are prefixes of this run", fontsize=8)
for k in (16, 30):
    hx.axvline(k + 0.5, color="#222222", lw=0.9, ls=":")
    hx.text(k + 1.0, -0.8, f"K={k}", fontsize=6.5, ha="left", va="bottom", color="#222222")
hx.set_ylim(len(rows) - 0.5, -2.6)
hx.text(K64 * 0.62, len(rows) * 0.55, "after ~loop 24: inert\n(1–3% of questions flip per loop)", fontsize=7, color="#555555", ha="center")
