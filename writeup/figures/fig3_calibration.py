...
def draw(ax, rates, title):
    ...
    line("fired_moved", ANSWER, "-", "readout showed a swap — answer moved")
    line("fired_still", RIVAL, "-", "readout showed a swap — answer did not move")
    line("null", CONTROL, "--", "random pair swapped", marker="")
    ax.set_xlabel("blend strength"); ax.set_ylim(-0.03, 1.03); ax.set_title(title, loc="left", fontsize=9, pad=10)
    ax.set_xticks(xs)
